...
"Done" keys off the `running` transition (busy → idle) — not `SessionSummary.completed`, which [the done-dot note](2026-08-06-session-completed-done-dot.md) arms only for sessions that are not the selected one, so it never fires for the session the operator is watching. "Done" is suppressed when the session stopped in order to ask for an interaction, so a single transition does not stack a "Done" and a "Waiting for approval" toast.
