...
import { homedir, platform as osPlatform, release as hostKernelRelease } from 'node:os'
...
export interface OpenInAppInternals {
  ...
  /** Kernel release for WSL detection; the opener's host default applies when absent. */
  osRelease?: string
...
export interface ResolvedInternals {
  platform: NodeJS.Platform
  /** Kernel release for WSL detection, defaulted once here. */
  osRelease: string
  ssh: boolean
  applicationRoots: readonly string[]
  env: Readonly<Record<string, string | undefined>>
...
export function resolveInternals(internals: OpenInAppInternals): ResolvedInternals {
  ...
  return {
    ...
    osRelease: internals.osRelease ?? hostKernelRelease(),
